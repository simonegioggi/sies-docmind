---
uniqueName: siut-sies-ar-17-20240802-architetturasies
displayName: "SIUT SIES AR 1.7 20240802 Architettura SIES"
category: "GENERAL"
tags: []
---
...
| 1.5 | 17/05/2023 | Revisione | Modificato il paragrafo 3.5.2 |
| 1.6 | 14/02/2024 | Revisione | Modificato il paragrafo 3.5.2; Aggiunto il paragrafo 3.5.3 |
| 1.7 | 02/08/2024 | Revisione | Aggiunto il paragrafo3.6 |
...
2Introduzione6
3Disegno generale del sistema7
3.1Interconnessione tra nodi SIES8
3.2SIES-NSC8
3.3PST vs SIUS AVVOCATURA9
3.4SIUS AVVOCATURA vs SISTEMI SIES10
3.5PagoPA/PST - SIES11
3.5.1Configurazione del certificato di comunicazione12
3.5.2Batch di verifica stato del Pagamento13
3.5.3Cruscotto per funzionalità Batch15
3.6BATCH DNA17
4Architettura del SISTEMA SIES18
5Architettura SIUS AVVOCATURA20
6Generazione dei report con WindWard21
7Software di base (Stack)26
8Librerie utilizzate27
9WEB Service29
10Start-Stop Application Server30
...
I flussi informativi che coinvolgono il sistema SIES ed il sistema del Casellario sono diversi:
...
Il batch interrogherà PagoPA per verificare lo stato dei pagamenti dei bollettini presenti sulla tabella “BOLLETTINO_PAGOPA” per i quali non risulta ancora pagato il bollettino (“DATA_AVV_PAGAMENTO”isnull e “STATO_PAGAMENTO=NP”) e per i quali le date “DATA_SCADENZA”, “DATA_ULTIMO_CONTROLLO” e  “DATA_GENERAZIONE_BOLLETTINO” soddisfino le condizioni sotto descritte.
...
SI (es. 15 e 10)NOgeneratiDaGiorni e controllarePerGiorni sono valorizzati?Controlla i bollettini scaduti da 3 giorni o che scadranno tra 3 giorni oppure quelli senza data scadenzaa partire dal quindicesimo giorno dalla loro generazione e per unmassimo di 10 giorniControlla i bollettini scaduti da 3 giorni o che scadranno tra 3 giorniSI (es. 3)SI (es. 5)SI (es. 15 e 10)NOFINEControlla solo i bollettini che NON sono stati controllati negli ultimi 5 giornicontrollateDaGiorni è valorizzato?NOControlla i bollettini a partire dal quindicesimo giorno dalla loro generazione e per unmassimo di 10 giorniNOgeneratiDaGiorni e controllarePerGiorni sono valorizzati?inScadenzaTraGiorni è valorizzato?INIZIO
...
Se un bollettino risulta pagato il batch aggiorna lo stato del pagamento e la data avvenuto pagamento.
...
Il batch sviluppato ha il compito di eliminare le tracce delle attività degli utenti della DNA. In generale le attività degli utenti SIES sono tracciate nei seguenti punti:

- file di log applicativi;
- tabella LOG_ATTIVITA;
- tabella MESAGGIO;
- dati di sistema.

Per i primi due punti sono stati aggiunti due filtri uno a livello di log (logback.xml)e uno a livello applicativo per scartare tutte le tracciature provenienti da attività di utenti dell'ufficio DNA.
Per gli ultimi due punti è stato sviluppato un batch che esegue periodicamente la pulizia dei record della tabella MESSAGGIO dove il mittente o il destinatario è un utente/ufficio della DNA. In questo modo verranno eliminate la tracce delle ricerche effettuate. Il batch si occuperà inoltre di verificare se un fascicolo SIEP, di fuori distretto, risulta essere stato preso in carico esclusivamente da un utente della DNA e in questo caso provvede alla cancellazione dei dati del fascicolo dal distretto.

L'attivazione e la schedulazione del batch avverranno tramite tre parametri presenti nel file f3b.properties:
DNASchedulerEnabled=true
DNACodUfficio = 05809100302
DNACronExpression=0 0 5 ? * 1

DNASchedulerEnabled: indica se attivare o no il batch,True per abilitare il batch/False per disabilitare il batch;
DNACodUfficio: codice dell'ufficio DNA per applicare i filtri di cancellazione;
DNACronExpression: Valore espresso attraverso la sintassi crontab like. Di default è stata inserita la stringa “0 05? * 1” che rappresenta una schedulazione settimanale la domenica (ultimo parametro 1) alle 05:00.
...
Come si evince dal diagramma, il nodo SIES è realizzato con architettura web threetier i cui principali elementi funzionali sono costituiti:
...
Per l’implementazione del pattern MVC viene utilizzato il framework F3B : Framework Bull Building Blocks.
...
L’applicativo SIUS utilizza una architettura web threetier.
...
<TipoUfficio>PROCURA DELLA REPUBBLICA PRESSO IL TRIBUNALE ORDINARIO</TipoUfficio>
    ...
    <Evento>
        ...
        <Notifica>
            ...
            <Ufficio>
                ...
                <Indirizzo>xxxyyy, 1234567890</Indirizzo>
                <Telefono>012 - 34567890</Telefono>
                <Fax>012 - 345678901</Fax>
    ...
    <Soggetto>
        ...
        <Cognome>cccccccccc</Cognome>
        <Nome>nnnnnnnnnn</Nome>
        <CodOperatoreInserimento>PMVE</CodOperatoreInserimento>
        <DataInserimento>15-01-2004</DataInserimento>
        <CodUfficioInserimento>02704202104</CodUfficioInserimento>
        ...
        <NomeMadre>mmmmmmmmmm</NomeMadre>
        <CognomeMadre>bbbbbbbbbb</CognomeMadre>
        <CodStatoNascita>039</CodStatoNascita>
        <DataNascita>01-01-1000</DataNascita>
        <AttoNascita>9988899988</AttoNascita>
        <CodFiscale/>
        <AnnoNascita>1000</AnnoNascita>
...
Sistema Operativo: RedHat Enterprise Linux 6.4 a 64bit
...
“ricercaAvvisi” è il web service per laricerca degli avvisi per avvocato.
...
Fermare il server jboss tramite il comando: “service jboss stop” e controllare l’avvenuta esecuzione dell’istruzione tramite il comando: “service jboss status”. Per le verifiche si può far riferimento al file di log /opt/jboss-eap-6.4/standalone/server.log

Start

Aprire una shell linux sul server SIES e loggarsi come utente root
Avviare il server jboss tramite il comando “service jboss start” e controllare l’avvenuta esecuzione dell’istruzione tramite il comando: “service jboss status”. Per le verifiche si può far riferimento al file di log /opt/jboss-eap-6.4/standalone/server.log
