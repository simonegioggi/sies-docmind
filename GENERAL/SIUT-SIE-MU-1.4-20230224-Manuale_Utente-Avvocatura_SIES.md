---
uniqueName: siut-sie-mu-14-20230224-manualeutente-avvocaturasi
...
| --- |
| Sistema di Consultazione Procedimenti ed Avvisi SIUS |
...
| 1.3 | 14/05/2020 | Revisione | Revisione a seguito dei ticket n° 20200513012 e n° 20200513016:Par. 1.1: Esplicitate le modalità di accesso al sistema tramite PST e come quest’ultimo è responsabile di tracciare il CF per passarlo in input al sistema di Consultazione;Par. 2.1: Eliminate le diciture in cui si specificava che l’utente autenticato sul PST fosse esclusivamente un avvocato. Ciò perché il PST, in fase di autenticazione non effettua nessun controllo sul ruolo dell’utente. Inoltre sono state specificate le regole di validità generale per la consultazione sul SIUS;Par. 3.1: Rettificati i pre-requisiti di accesso al sistema di Consultazione eleminando le diciture in cui si specificava che l’utente autenticato sul PST fosse esclusivamente un avvocato.Il documento è stato revisionato nella sua completezza in considerazione del fatto che il PST non garantisce che l’utente autenticato sia necessariamente un avvocato. In tal senso, quindi, dove utilizzato il riferimento ad utente avvocato è stata effettuata la sostituzione con utente generico. |
| 1.4 | 24/02/2023 | Revisione | Revisione a seguito del Ticket#20230201017 - Anomalia Sies:Par. 2.1 nella sezione visibilità Avvisi, “Validazione del Deposito di un’Ordinanza di Rinvio Udienza” |
...
| Francesco Rosati | RTI |  | Responsabile Manutenzione CorrettivaReferente Qualità e Sicurezza |
| Andrea Salvaggio | RTI |  | Responsabile Progetto Sistema UnitarioReferente Tecnico |
...
1.2.Glossario6
1.2.1.Definizioni6
1.2.2.Acronimi e abbreviazioni7
2.Avvocatura - SIES9
2.1.Scopo dell’applicativo9
3.ACCESSO Avvocatura - SIES12
3.1.Login12
4.HOME PAGE13
5.RICERCHE14
5.1.RICERCA DI PROCEDIMENTO DI SORVEGLIANZA PER ESTREMI PROCEDIMENTO14
5.1.1.Pagina di ricerca per estremi del procedimento14
5.1.2.Dettaglio Procedimento Sorveglianza15
5.1.2.1.Dettaglio Ordinanza19
5.1.2.2.Dettaglio Decreto21
5.1.2.3.Dettaglio Decreto di citazione (fissazione udienza)22
5.2.RICERCA DATI SOGGETTO24
5.2.1.Pagina di ricerca soggetti con procedimenti di sorveglianza24
5.2.2.Elenco Procedimento Soggetto26
6.CONSULTAZIONE28
6.1.CONSULTAZIONE AVVISI28
6.1.1.Pagina di ricerca Avvisi28
6.1.2.Risultato Elenco Avvisi29
...
Esso si presenta come Applicazione Web che sarà accessibile tramite la Loginposta nella home page del PST (Portale Servizi Telematici).
...
- Decreto di fissazione udienza (se validato);
...
- Gli Avvisivisibili all’utente richiedente sono relativi ai seguenti eventi:
...
La prima tipologia di ricerca, ovveroRicerca Estremi Procedimento, è una ricerca basata sull’ Anno del Procedimento ed il Numero di Procedimento SIUS. All’utente che farà accesso a questa tipologia sarà prospettata una maschera in cui devono essere obbligatoriamente inseriti l’anno del procedimento, il numero di procedimento, il tipo ufficio, il distretto di interesse e la sede (quest’ultimo dato è obbligatorio solo per uffici UDS). Il risultato della ricerca sarà una pagina di dettaglio del procedimento individuato.

La seconda tipologia di ricerca ovveroRicerca Dati Soggetto è una ricerca basata sui dati anagrafici del Soggetto.
...
Il sistema di autenticazione PST, in fase di accesso al Sistema di Consultazione, devefornire, come informazione da passare nella request, il codice fiscale dell’utente loggato su PST.
...
Coniltermine"Home Page"si intendeilprimoquadroacuisiaccedeappena effettuatol’accesso otramitelapressionesul logo “Servizi Online - Sistema di Consultazione procedimenti ed avvisi Sius”presentesulla parte sinistra dellabarra degli strumenti.

Figura 2– Home Page

Nella barra del titolo sulla destra è indicata, tramitelink,l’utenza ed il codice fiscale con cui è avvenuto l’accesso.Cliccando sul suddetto linkè possibile effettuare la Disconnessione/Logout dall’applicazione:
...
In Ricerchel’utente che accede all’applicazione avrà la possibilità di consultare in tempo reale, tutti i procedimenti del sistema SIUS in cui lui risulta difensore assegnatario.
...
Il pulsante “Cerca” avvia la ricerca del procedimento SIUScon i parametri immessi in maschera.
...
Cliccando sull’icona della frecciaè possibile espandere la sezione e visualizzare i dati contenuti.
...
In fondo alla maschera è presente il pulsante ‘Torna al Procedimento’:
Tramite questo pulsante, dalla pagina di dettaglio del decreto di fissazione udienzasi torna alla pagina di dettaglio del procedimento.
...
d) Comune di Nascita - comune di Nascita del Soggetto da ricercare; Obbligatorio solo per i soggetti nati in Italia; Lista di selezionecon la scelta dei comuni;

e) Stato di Nascita – stato di Nascita del Soggetto da ricercare; Obbligatorio; Lista di selezionecon la scelta degli Stati;
...
Il pulsante “Cerca” avvia la ricerca per dati soggettocon i parametri immessi in maschera.
...
Il risultato della ricerca è presentato in una mascherain cui, nella parte superiore sono riepilogati i parametri di ricerca, mentre nella parte inferiore mostra l’elenco dei soggetti trovati in base ai criteri di ricerca inseriti a sistema.
...
Il valore riportato in corrispondenza del campo ‘Tipo Provvedimento’è unlink che permette all’utente di visualizzare il dettaglio del provvedimento. In particolare, il dettaglio può far riferimento al dettaglio di un decreto di citazione (vd. par.5.1.2.3), al dettaglio di un decreto (vd. par.5.1.2.2) oppure al dettaglio di un’ordinanza (vd. par.5.1.2.1).
