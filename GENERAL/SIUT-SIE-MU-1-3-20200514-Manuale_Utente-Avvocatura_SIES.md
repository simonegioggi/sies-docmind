---
uniqueName: siut-sie-mu-1-3-20200514-manualeutente-avvocaturas
displayName: "SIUT SIE MU 1 3 20200514 Manuale Utente Avvocatura SIES"
category: "GENERAL"
tags: []
---

# SIUT-SIE-MU-1.3-20200514-Manuale_Utente-Avvocatura_SIES

> **File originale:** `SIUT-SIE-MU-1.3-20200514-Manuale_Utente-Avvocatura_SIES.pdf`  
> **Tipo:** PDF
...
SIUT-SIE-MU-1.3-20200514-Manuale Utente-Avv-SIUS 
Ver. 1.3 del 14/05/2020
Pag. 2/30 
...
responsabile di tracciare il CF per passarlo in 
input al sistema di Consultazione; 
• 
Par. 2.1: Eliminate le diciture in cui si 
specificava che l’utente autenticato sul PST 
fosse esclusivamente un avvocato. Ciò perché 
il PST, in fase di autenticazione non effettua 
nessun controllo sul ruolo dell’utente. Inoltre 
sono state specificate le regole di validità 
generale per la consultazione sul SIUS; 
• 
Par. 3.1: Rettificati i pre-requisiti di accesso al 
sistema di Consultazione eleminando le 
diciture in cui si specificava che l’utente 
autenticato sul PST fosse esclusivamente un 
avvocato. 
Il documento è stato revisionato nella sua completezza in 
considerazione del fatto che il PST non garantisce che
l’utente autenticato sia necessariamente un avvocato. In
tal senso, quindi, dove utilizzato il riferimento ad utente 
avvocato è stata effettuata la sostituzione con utente 
generico. 
...
Questo documento illustra le modalità operative per l’utilizzo del sistema Avvocatura – SIES che espone 
le funzionalità per la consultazione dei procedimenti di Sorveglianza e degli Avvisi ad essi inerenti. 
...
dell’utente che fa accesso al sistema di Consultazione Avvocatura SIES. Il sistema lavora esclusivamente 
sul valore del Codice Fiscale, il quale è utilizzato per le ‘interrogazioni’ al SIUS. Il sistema SIUS restituisce 
i dati ricercati dall’utente solo se l’utente è registrato al suo interno quale avvocato attivo ed è associato 
come difensore ad un procedimento SIUS. 
1.2. Glossario 
1.2.1. Definizioni 
...
A seguito della ‘Login’ su PST, eseguita secondo le modalità esplicitate al par. 1.1, il portale mostrerà un 
...
l’individuazione dei procedimenti del SIUS, pertanto il Codice Fiscale è un dato obbligatorio che il sistema 
di Consultazione si aspetta in input dal sistema di autenticazione PST. 
...
1. Decreto di fissazione udienza (anche non validato); 
...
Con la Consultazione l’utente avrà la possibilità di consultare gli Avvisi che il sistema SIUS genera in fase 
di validazione di un decreto di citazione (fissazione udienza) oppure al momento del deposito 
...
cui difensore è l’utente che ha effettuato l’accesso all’applicazione. L’utente richiedente deve essere 
abbinato al fascicolo SIUS di interesse in qualità di avvocato attivo. 
