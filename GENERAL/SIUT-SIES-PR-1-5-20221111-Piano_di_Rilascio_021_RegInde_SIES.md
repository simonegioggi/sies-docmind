---
uniqueName: siut-sies-pr-1-5-20221111-pianodirilascio021regind
displayName: "SIUT SIES PR 1 5 20221111 Piano di Rilascio 021 RegInde SIES"
category: "GENERAL"
tags: []
---

# SIUT-SIES-PR-1.5-20221111-Piano_di_Rilascio_021_RegInde_SIES

> **File originale:** `MEV/SCHEDA_021/Docs/SIUT-SIES-PR-1.5-20221111-Piano_di_Rilascio_021_RegInde_SIES.docx`  
> **Tipo:** DOCX
...
| 1.5 | 11/11/2022 | Sesta Emissione | Modificati i riferimenti al documento di bonifica difensori ed inglobato il documento di aggiornamento SIES da tabelle DGSIA; corretta la versione di installazione del SIES |
...
6.3	Procedura di Aggiornamento Tabella SIES da Tabella fissa DGSIA	10
6.3.1	Attività di installazione ed esecuzione procedura batch Aggiornamento Tabella SIES da Tabella fissa DGSIA	10
6.3.2	Procedura Aggiornamento Tabelle SIES - STEP 1	11
6.3.3	Procedura Aggiornamento Tabelle SIES - STEP 2	13
6.4	Installazione applicazione	13
6.4.1	Deploy Applicazione	13
6.5	Attività di configurazione	14
6.6	Attività di post-installazione	14
...
Gli interventi in oggetto sono rilasciati nell’ambito della release 12.4.24.0-MEV_2019_021 di SIES.
...
# Generalità
Il presente documento descrive il piano di rilascio del software relativo alla MEV-2019_021 di SIES, contenente l’integrazione della suddetta MEV con la versione SIES 12.4.24.0.
...
| --- | --- | --- | --- |
| Portale fornitura | SIUT > 07 - MEV > 2019_021_SIES_Reginde > 05_Verifica di Conformita' | 1.5 | 11/11/2022 |
| Note-osservazioni |  |  |  |
...
| documentazione.zip | Documentazione | SIUT-SIES-PR-1.5-20221111-Piano_di_Rilascio_021_RegInde_SIES.pdf
...
# Installazione
## Prerequisiti
Per l’installazione della release SIES oggetto del presente rilascio è necessario aver installato la precedente release 12.4.24.0.
...
## Attività di installazione ed esecuzione procedura batch Aggiornamento Tabella SIES da Tabella fissa DGSIA
Copiare il file aggiorna_db.zip in una qualsiasi cartella e scompattarlo. Il sistema creerà la cartella aggiorna_db;
...
Posizionarsi nel percorso /home/oracle/MEV-2019_21/aggiorna_db/; lanciare sequenzialmente i comandi, indicati nel par. 6.3.2.
## Procedura Aggiornamento Tabelle SIES - STEP 1
Verificare tramite il comando pwd di trovarsi nel percorso /home/oracle/MEV-2019_21/aggiorna_db/ ed eseguire il comando:
./aggiorna_db.sh
Dettaglio del file:
...
Aggiornamento_CODICI_SIES_NSC_NAZIONE_esercizio.sql = Lo script cancella dalla tabella
...
Insert_FUNZIONE-FUNZIONE_PROFILO.sql = Lo script inserisce nelle tabelle FUNZIONE e FUNZIONE_PROFILO i records necessari ad abilitare le nuove funzionalità necessarie alla ricerca ed alla gestione dell’Avvocato su ReGIndE ed alla ricerca e gestione del comune e provincia di nascita (nuova struttura delle tabella COMUNE e CODICI_SIES_NSC).
Inserimento_CODICI_SIES_NSC_NAZIONE_NO_Cod_Univoco.sql = Lo script inserisce nella tabella
CODICI_SIES_NSC i 32 records aventi CO_DOMAIN = ‘NAZIONE’ e che hanno una corrispondenza in entrambe le tabelle degli stati [CG_REF_CODES (RV_DOMAIN = ‘NAZIONE’ di SIES, DC_TAB_STATO_ESTERO di NSC] ma non hanno nella tabella di NSC valorizzato il codice univoco.
...
verificare che nel file /home/oracle/MEV-2019_21/aggiorna_db/log/Log_Aggiorna_DB.log non siano riportati errori oracle;
...
Per verificare l’aggiornamento effettuato al dodicesimo punto eseguire quanto di seguito
SELECT COUNT(*) FROM CODICI_SIES_NSC WHERE CO_DOMAIN = 'NAZIONE'
il risultato deve essere 236.

In caso di esito positivo delle su riportate attività LE TABELLE DI BACKUP COMUNE_SXALLTF, CG_REF_CODES_SXALLTF e CODICI_SIES_NSC_SXALLTF NON DEVONO ESSERE CANCELLATE MA VANNO MANTENUTE PER ALMENO 6 MESI, PER PERMETTERE EVENTUALI ATTIVITÀ DI VERIFICA.
