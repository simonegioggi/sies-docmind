---
uniqueName: siut-sies-mg-1-2-20220124-aggiornamentosiesdatabel
displayName: "SIUT SIES MG 1 2 20220124 Aggiornamento SIES da Tabelle DGSIA 021 RegInde SIES"
category: "GENERAL"
tags: []
---

# SIUT-SIES-MG-1.2-20220124-Aggiornamento_SIES_da_Tabelle_DGSIA_021_RegInde_SIES

> **File originale:** `MEV/SCHEDA_021/Docs/consegna docx/ORIGINALS/SIUT-SIES-MG-1.2-20220124-Aggiornamento_SIES_da_Tabelle_DGSIA_021_RegInde_SIES.docx`  
> **Tipo:** DOCX
...
3.1.1	Aggiornamento tabella COMUNE	8
3.1.2	Aggiornamento Dominio PROVINCIA nella CG_REF_CODES	14
3.1.3	Aggiornamento Dominio NAZIONE nella CG_REF_CODES	16
3.1.4	Aggiornamento Dominio REGIONE nella CG_REF_CODES	18
3.1.5	Aggiornamento tabella CODICI_SIES_NSC   per   CO_DOMAIN = ‘COMUNE’	18
3.1.6	Aggiornamento tabella CODICI_SIES_NSC   per   CO_DOMAIN = ‘NAZIONE’	20
3.1.7	Aggiornamento tabelle COMUNI e STATI   di SIES-AVVOCATURA	23
3.1.8	Rilascio per messa in esercizio	25
...
# 2 	Generalità
Le attività descritte di seguito sono state effettuate per ottenere le nuove tabelle COMUNE, CG_REF_CODES, relativamente ai Domini PROVINCIA, REGIONE e NAZIONE, e CODICI_SIES_NSC, relativamente ai Domini COMUNE e NAZIONE, che saranno rilasciate già aggiornate con i seguenti scripts:
...
Aggiorna_ AGGIORNA_COMUNI_AVVOCATURA.prc

Aggiorna_TABELLA_STATI_SIES_AVVOCATURA.sql

Le attività riportate di seguito possono essere rieseguite, partendo dalle attuali tabelle di esercizio, per la verifica della correttezza delle attività fatte per giungere all’ottenimento delle nuove tabelle/nuovi domini.

# 3 	Descrizione delle Attività
Le attività descritte nel paragrafo 3.1 si articolano su scripts.sql e procedures plsql, contenuti nel pacchetto aggiornaTF_db.zip, che va estratto ottenendo la cartella ..\aggiornaTF_db\aggiornaTF.
...
(*) poichè nella tabella AVVOCATO_REGINDE_18112020 rilasciata le suddette colonne sono già valorizzate prima di procedere all’esecuzione della suddetta procedure, bisogna azzerare il valore delle colonne  eseguendo la seguente query
...
## Rilascio per messa in esercizio
...
che esegue una copia di backup delle tabelle COMUNE, CG_REF_CODES e CODICI_SIES_NSC nelle tabelle COMUNE_SXALLTF, CG_REF_CODES_SXALLTF e CODICI_SIES_NSC_SXALLTF e delle tabelle COMUNI, STATI di SIES-Avvocatura nelle tabelle COMUNI_SXALLTF e STATI_SXALLTF.
...
eseguire lo script 	Aggiornamento_TABELLA_COMUNE_esercizio.sql

che cancella i records preesistenti 8112 e ne inserisce 13327.

eseguire lo script	Aggiornamento_CG_REF_CODES_PROVINCIA_esercizio.sql
...
eseguire lo script	Insert_REGIONE.sql

inserisce un nuovo record (LUOGO_SCONOSCIUTO) nella tabella CG_REF_CODES per RV_DOMAIN = ‘REGIONE’

eseguire lo script	Aggiornamento_CG_REF_CODES_NAZIONE_esercizio.sql
...
eseguire lo script	Aggiornamento_CODICI_SIES_NSC_COMUNE_esercizio.sql
...
eseguire lo script	Aggiornamento_CODICI_SIES_NSC_NAZIONE_esercizio.sql
che cancella dalla tabella CODICI_SIES_NSC i records aventi CO_DOMAIN = ‘NAZIONE’ e carica i nuovi records. Il dominio conterrà 205 records a fronte degli 201 precedenti.

eseguire lo script	Alter_COMUNI _STATI.sql
...
eseguire la procedure	AGGIORNA_COMUNI_AVVOCATURA.prc

che cancella i records preesistenti 8112 e ne inserisce 13327.

eseguire lo script 	Aggiorna_TABELLA_STATI_SIES_AVVOCATURA.sql che cancella i records preesistenti 200 e ne inserisce 269.


In caso di anomalie che dovessero verificarsi nel corso degli aggiornamenti è possibile restorare la tabella COMUNE e/o i Domini delle tabelle CG_REF_CODES e CODICI_SIES_NSC secondo il seguente schema:
...
| FUNZIONE, FUNZIONE_PROFILO |  | Restore_Funzione_FunzioneProfilo.sql |