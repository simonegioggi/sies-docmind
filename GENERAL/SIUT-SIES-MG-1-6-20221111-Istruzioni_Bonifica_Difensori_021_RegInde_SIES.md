---
uniqueName: siut-sies-mg-1-6-20221111-istruzionibonificadifens
displayName: "SIUT SIES MG 1 6 20221111 Istruzioni Bonifica Difensori 021 RegInde SIES"
category: "GENERAL"
tags: []
---

# SIUT-SIES-MG-1.6-20221111-Istruzioni_Bonifica_Difensori_021_RegInde_SIES

> **File originale:** `MEV/SCHEDA_021/Docs/SIUT-SIES-MG-1.6-20221111-Istruzioni_Bonifica_Difensori_021_RegInde_SIES.docx`  
> **Tipo:** DOCX
...
| --- | --- | --- |
| RIF1. | SIUT-SIES-PR-1.5-20221111-Piano_di_rilascio_021_RegInde_SIES | Piano di rilascio |
...
Copiare la cartella bonifica_db nella cartella /home/oracle//.
...
Posizionarsi nel percorso /home/oracle//bonifica_db/ e creare la cartella log tramite il comando mkdir log e, poi, dare i permessi tramite il comando chmod 777 log.
Posizionarsi nel percorso /home/oracle//bonifica_db/bonifica/; lanciare il comando, sostituendo xx con la sigla del distretto, come già indicato al punto 7:
...
Posizionarsi nel percorso /home/oracle//bonifica_db/; lanciare sequenzialmente i comandi, indicati nel par. 3.2 Procedura Bonifica Difensori SIES
...
Verificare che nel file /home/oracle//bonifica_db/log/LOG_Bonifica_AVV_1.log non siano riportati errori oracle;
...
Verificare che nel file /home/oracle//bonifica_db/log/LOG_Bonifica_AVV_3.log non siano riportati errori oracle;
...
verificare che nel file /home/oracle//bonifica_db/log/LOG_Bonifica_AVV_5.log non siano riportati errori oracle;
...
Con le seguenti queries è possibile estrarre l’elenco dei procedimenti pendenti dei diversi ambienti dalle tabelle PRE e POST.

Il numero dei procedimenti pendenti SIEP deve essere = SUM(FAS_SIEP):

SELECT ID_AVVOCATO, COD_FISCALE, COGNOME, NOME, FORO, AMBIENTE, ID_FASCICOLO_SIEP, CHIAVE_ANNO, CHIAVE_PROGR, COD_TIPO_UFF, DESCRCOMUNE, FLAG_REGINDE
FROM POST_XBA_AVVOCATI_CON_PENDENZE WHERE COD_FISCALE = 'XXXXXXXXXXXXXXXX' AND AMBIENTE = 'SIEP' ORDER BY  ID_AVVOCATO, ID_FASCICOLO_SIEP

SELECT ID_AVVOCATO, COD_FISCALE, COGNOME, NOME, FORO, AMBIENTE, ID_FASCICOLO_SIEP, CHIAVE_ANNO, CHIAVE_PROGR, COD_TIPO_UFF, DESCRCOMUNE
FROM PRE_XBA_AVVOCATI_CON_PENDENZE WHERE COD_FISCALE = 'XXXXXXXXXXXXXXXX' AND AMBIENTE = 'SIEP' ORDER BY  ID_AVVOCATO, ID_FASCICOLO_SIEP

Il numero dei procedimenti pendenti SIUS deve essere = SUM(FAS_SIUS)

SELECT ID_AVVOCATO, COD_FISCALE, COGNOME, NOME, FORO, AMBIENTE, ID_FASCICOLO_SIUS, CHIAVE_ANNO, CHIAVE_PROGR, COD_TIPO_UFF, DESCRCOMUNE, FLAG_REGINDE
FROM POST_XBA_AVVOCATI_CON_PENDENZE WHERE COD_FISCALE = 'XXXXXXXXXXXXXXXX' AND AMBIENTE = 'SIUS' ORDER BY  ID_AVVOCATO, ID_FASCICOLO_SIUS

SELECT ID_AVVOCATO, COD_FISCALE, COGNOME, NOME, FORO, AMBIENTE, ID_FASCICOLO_SIUS, CHIAVE_ANNO, CHIAVE_PROGR, COD_TIPO_UFF, DESCRCOMUNE
FROM PRE_XBA_AVVOCATI_CON_PENDENZE WHERE COD_FISCALE = 'XXXXXXXXXXXXXXXX' AND AMBIENTE = 'SIUS' ORDER BY  ID_AVVOCATO, ID_FASCICOLO_SIUS

Il numero dei procedimenti pendenti SIGE deve essere = SUM(FAS_SIGE)

SELECT ID_AVVOCATO, COD_FISCALE, COGNOME, NOME, FORO, AMBIENTE, ID_FASCICOLO_SIGE, CHIAVE_ANNO, CHIAVE_PROGR, COD_TIPO_UFF, DESCRCOMUNE, FLAG_REGINDE
FROM POST_XBA_AVVOCATI_CON_PENDENZE WHERE COD_FISCALE = 'XXXXXXXXXXXXXXXX' AND AMBIENTE = 'SIGE' ORDER BY  ID_AVVOCATO, ID_FASCICOLO_SIGE

SELECT ID_AVVOCATO, COD_FISCALE, COGNOME, NOME, FORO, AMBIENTE, ID_FASCICOLO_SIGE, CHIAVE_ANNO, CHIAVE_PROGR, COD_TIPO_UFF, DESCRCOMUNE
FROM PRE_XBA_AVVOCATI_CON_PENDENZE WHERE COD_FISCALE = 'XXXXXXXXXXXXXXXX' AND AMBIENTE = 'SIGE' ORDER BY  ID_AVVOCATO, ID_FASCICOLO_SIGE
## 3.2.6 STEP 6
...
Verificare che nel file /home/oracle//bonifica_db/log/LOG_Bonifica_AVV_6.log non siano riportati errori oracle;
...
dalla lista selezionare a campione dei procedimenti SIUS e verificare con accesso al database, che al procedimento risulti collegato l’avvocato certificato. Per esempio:
select a.flag_reginde from avvocato a where a.id_avvocato in (SELECT max(t.avv_id_avvocato_new) FROM XBA_VERIF_AVVISI_AVVOCATO t);
...
dalla lista selezionare a campione dei procedimenti Sige e verificare con accesso al database, che al procedimento risulti collegato l’avvocato certificato. Per esempio:
select a.flag_reginde from avvocato a where a.id_avvocato in (SELECT max(t.avv_id_avvocato_new) FROM XBA_VERIF_PARTI_UDIENZA_DIF t);
...
dalla lista selezionare a campione dei records e verificare con accesso al database, l’avvenuto aggiornamento dell’Identificativo dell’avvocato certificato. Per esempio:
select a.flag_reginde from avvocato a where a.id_avvocato in (SELECT max(t.avv_id_avvocato_new) FROM XBA_VERIF_STORICO_AVVOCATO t);
...
dalla lista selezionare a campione dei procedimenti SIEP e verificare con accesso al database, che al procedimento risultino collegati avvocati certificati. Per esempio:
select a.flag_reginde from avvocato a where a.id_avvocato in (SELECT max(t.avv_id_avvocato_new) FROM XBA_VERIF_NUOVA_ISTANZA t);
...
Alla fine di queste attività di aggiornamento, passare paragrafo 6.3 del documento “SIUT-SIES-PR-1.5-20221111-Piano_di_rilascio_021_RegInde_SIES.pdf”.
## 3.2.7 STEP 7
