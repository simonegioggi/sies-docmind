---
uniqueName: siut-sies-mg-1-6-20220915-istruzionibonificadifens
displayName: "SIUT SIES MG 1 6 20220915 Istruzioni Bonifica Difensori 021 RegInde SIES rev"
category: "GENERAL"
tags: []
---

# SIUT-SIES-MG-1.6-20220915-Istruzioni_Bonifica_Difensori_021_RegInde_SIES-rev

> **File originale:** `MEV/SCHEDA_021/Docs/SIUT-SIES-MG-1.6-20220915-Istruzioni_Bonifica_Difensori_021_RegInde_SIES-rev.docx`  
> **Tipo:** DOCX
...
- che il totale dei records per FORLÌ-CESENA = somma dei records riportati per FORLI’, CESENA,
FORLI’ CESENA, FORLÌ-CESENA della Tabella pre, in cui possono essere presenti solo parte delle precedenti descrizioni;
...
1 e 2)
Eseguire nell’ordine le seguenti 4 queries:

SELECT count(*) "Totale avv. con proc. pendenti" FROM XBA_AVVOCATI_CON_PENDENZE;

SELECT count(*) "Totale avv. certificabili" FROM SIESRM.XBA_AVVOCATI_CON_PENDENZE WHERE FLAG_REGINDE = 'SI';

SELECT count(*) "Tot.avv. assoc. a certif." FROM SIESRM.XBA_AVVOCATI_CON_PENDENZE WHERE FLAG_REGINDE = 'NO' AND ID_AVV_CERT_REGINDE IS NOT NULL;

SELECT count(*) "Tot.avv. non certificabili" FROM SIESRM.XBA_AVVOCATI_CON_PENDENZE WHERE FLAG_REGINDE = 'NO' AND ID_AVV_CERT_REGINDE IS NULL;
...
per raffrontare i dati post e pre bonifica, accedere alla tabella POST_XBA_AVVOCATI_CON_PENDENZE, ordinandola per codice fiscale, selezionare un avvocato ed eseguire la query cambiando XXXXXXXXXXXXXXXX con il codice fiscale dell’avvocato selezionato
...
Alla fine di queste attività di aggiornamento, passare al punto 2 del paragrafo 6.1 del documento “.docx”.
## 3.2.7 STEP 7
