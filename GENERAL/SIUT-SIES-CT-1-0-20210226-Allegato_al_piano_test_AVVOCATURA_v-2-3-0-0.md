---
uniqueName: siut-sies-ct-1-0-20210226-allegatoalpianotestavvoc
displayName: "SIUT SIES CT 1 0 20210226 Allegato al piano test AVVOCATURA v 2 3 0 0"
category: "GENERAL"
tags: []
---

# SIUT-SIES-CT-1.0-20210226-Allegato_al_piano_test_AVVOCATURA_v.2.3.0.0

> **File originale:** `Avvocatura-SIES/RILASCIO_2.3.0.0/SIUT-SIES-CT-1.0-20210226-Allegato_al_piano_test_AVVOCATURA_v.2.3.0.0.xls`  
> **Tipo:** XLS
...
|  | Nome del file | SIUT-SIES-CT-1.0-20210226-Allegato-al-piano-test_AVVOCATURA_v.2.3.0.0.xls |
|  | Piano di test | SIUT-SIES-PT-1.0-20210226-Piano_dei_Test_AVVOCATURA_v.2.3.0.0.doc |
|  | Versione | 1.0 |
|  | Data | 44253.0 |
|  | Intervento | AVVOCATURA v.2.3.0.0 |
|  | Area Applicativa | Penale |
|  | Allegato al Piano dei Test |  |

## TabellaTest

| Codice Area |  |  | AVVOCATURA v.2.3.0.0 |  |  |  | Codifica  Piano di test |  | SIUT-SIES-CT-1.0-20210226-Allegato-al-piano-test_AVVOCATURA_v.2.3.0.0.xls | Versione | 1.0 |  |  |  |  |  |  |  |  |  |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
...
| SIUT | SIES | 202102010110.0 |  |  | 202102010110.0 | SIUS Avvocati, Logout |  | SIUS | SIUS - SIUS Avvocati, Logout | TF001.SIUS | Verifica corretta scrittura del file di debug.log |  |  |  | Verifica: verificare che effettuando il logout dall'applicazione il file di debug.log sia scritto correttamente e non contenga indicazioni di errori ma info nella parte relativa al logout. | 1.0 |  |  |  |  |
| SIUT | SIES | 20210205017.0 |  |  | 20210205017.0 | Errata visualizzazione esito provvedimenti elencati su SIUS-Avvocati |  | SIUS | SIUS - Errata visualizzazione esito provvedimenti elencati su SIUS-Avvocati | TF002.SIUS | Verifica corretta visualizzazione della colonna esito provvedimento |  |  |  | Verifica: verificare che l'elenco dei fascicoli ottenuti dalla ricerca per soggetto visualizzano correttamente la colonna esito provvedimento | 1.0 |  |  |  |  |
...
| Progetto / Obiettivo Software |  |  | AVVOCATURA v.2.3.0.0 |  |  |  |  |  |  |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Codifica Piano di test |  |  | SIUT-SIES-CT-1.0-20210226-Allegato-al-piano-test_AVVOCATURA_v.2.3.0.0.xls |  |  |  |  |  |  |
| ID caso di test | Suffisso Caso di test | Nome Caso di test
Tipo Step |  | Descrizione |  | Versione | Stima tempo | N.cond. di test | Esito |
| TF001.SIUS | 01 | Verifica corretta scrittura del file di debug.log |  | Verifica: verificare che effettuando il logout dall'applicazione il file di debug.log sia scritto correttamente e non contenga indicazioni di errori ma info nella parte relativa al logout. |  | 1.0 |  |  | OK |
...
|  | 1 | Azione | L'utente naviga all'interno dell'applicazione effettuando le ricerche e consultazioni di cui necessita |  |  |  |  |  |  |
|  | 2 | Azione | L'utente effettua il logout dal sistema di consultazione |  |  |  |  |  |  |
|  | V | Verifica | Verificare che il file di debug.log sia scritto correttamente e contenga nella parte relativa al logout dei logger.info quando si forniscono le informazioni sulla URI della request, sul nome del server, sul protocollo usato, sullo schema (http o https) utilizzato. |  |  |  |  |  |  |
| TF002.SIUS | 01 | Verifica corretta visualizzazione della colonna esito provvedimento |  | Verifica: verificare che l'elenco dei fascicoli ottenuti dalla ricerca per soggetto visualizzano correttamente la colonna esito provvedimento |  | 1.0 |  |  | OK |
|  | Step | Tipo | Istruzioni
(test) | Dati di input
(Associated Data) | Risultati attesi
(Expected Result) |  |  |  |  |
...
|  | 2 | Azione | L'utente ricerca un soggetto che abbia almeno un procedimento, imposta i criteri di ricerca per soggetto e preme il pulsante "Cerca". |  | Visualizzazione dati soggetto |  |  |  |  |
...
|  | V | Verifica | Verificare che venga prospettato correttamente il contenuto della colonna Esito Provvedimento. |  |  |  |  |  |  |
|  | 4 | Azione | L'utense seleziona tramite il link contenuto nella colonna N.SIUS un procedimento. |  | Visualizazzione pagina contenente il dettaglio del procedimento |  |  |  |  |
...
|  | V | Verifica | Verificare che venga prospettata correttamente la sezione Esiti con lo stesso contenuto visualizzato nella pagina Elenco Fascicoli dello step 3. |  |  |  |  |  |  |
