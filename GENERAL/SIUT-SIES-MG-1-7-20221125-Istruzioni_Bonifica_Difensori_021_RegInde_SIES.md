---
uniqueName: siut-sies-mg-1-7-20221125-istruzionibonificadifens
displayName: "SIUT SIES MG 1 7 20221125 Istruzioni Bonifica Difensori 021 RegInde SIES"
category: "GENERAL"
tags: []
---

# SIUT-SIES-MG-1.7-20221125-Istruzioni_Bonifica_Difensori_021_RegInde_SIES

> **File originale:** `MEV/SCHEDA_021/Docs/SIUT-SIES-MG-1.7-20221125-Istruzioni_Bonifica_Difensori_021_RegInde_SIES.docx`  
> **Tipo:** DOCX
...
| 1.6 | 11/11/2022 | Settima Emissione | Rivista intera struttura documento |
| 1.7 | 25/11/2022 | Settima Emissione | Rivista intera struttura documento |
...
| --- | --- | --- |
| RIF1. | SIUT-SIES-PR-1.6-20221125-Piano_di_rilascio_021_RegInde_SIES | Piano di rilascio |
...
In fase di esecuzione degli script, di seguito riportati, saranno chiesti alcuni parametri; di seguito un esempio di tale richiesta (dove xx è la sigla del distretto di appartenenza, per esempio: rm per Roma):
==================================================
...
Posizionarsi nel percorso /home/oracle/MEV-2019_21/bonifica_db/bonifica/; lanciare il comando, sostituendo xx con la sigla del distretto, come già indicato al punto 7:
...
PRE_XBA_CREA_AVV_CON_PENDENZE.prc = Crea Tabella PRE_XBA_AVVOCATI_CON_PENDENZE che conterrà i dettagli di tutti gli avvocati aventi procedimenti SIEP, SIUS e SIGE pendenti. Questa tabella strutturalmente è una immagine della tabella AVVOCATO, con l’aggiunta delle seguenti colonne:
...
Collegarsi al database con utenza siesxx (dove xx è la sigla del distretto di appartenenza, per esempio rm per Roma, per cui la stringa sarà siesrm):
...
Per verificare la correttezza dei numeri riportati nella PRE_XBA_AVVOCATI_CON_PENDENZE eseguire le seguenti queries, in cui bisogna sostituire il parametro ‘NN’ con il Codice del Distretto, che può essere individuato eseguendo la seguente query:

SELECT DESCRIZIONE FROM JMS_CODE WHERE DOMINIO = 'PROGR_BDI' AND
CODICE = 'YYYYYYY'

in cui ‘YYYYYYY’ deve essere sostituito con il nome del Distretto (es. ROMA, TORINO, PALERMO,…..)
...
JOIN AVVOCATO_FASCICOLO_SIEP afs ON (afs.AVV_ID_AVVOCATO = a.ID_AVVOCATO and SUBSTR(afs.FAS_SIE_ID_FASCICOLO_SIEP,-6, 2) = 'NN')
...
JOIN AVVOCATO_FASCICOLO_SIUS afs ON (afs.AVV_ID_AVVOCATO = a.ID_AVVOCATO and SUBSTR(afs.FAS_SIU_ID_FASCICOLO_SIUS,-6, 2) = 'NN')
...
JOIN AVVOCATO_FASCICOLO_SIGE afs ON (afs.AVV_ID_AVVOCATO = a.ID_AVVOCATO and SUBSTR(afs.FAS_SIGE_ID_FASCICOLO_SIGE,-6, 2) = 'NN')
...
select count(*) "Fascicoli SIEP pendenti" from AVVOCATO a JOIN AVVOCATO_FASCICOLO_SIEP afs   ON(afs.AVV_ID_AVVOCATO = a.ID_AVVOCATO AND SUBSTR(afs.FAS_SIE_ID_FASCICOLO_SIEP,-6, 2) = 'NN') JOIN FASCICOLO_SIEP fs ON (fs.ID_FASCICOLO_SIEP = afs.FAS_SIE_ID_FASCICOLO_SIEP and  fs.COD_STATO_FASCICOLO <> '01' )

SELECT count(*) FROM PRE_XBA_AVVOCATI_CON_PENDENZE pxacp WHERE AMBIENTE = 'SIUS'

deve essere uguale a

select count(*) "Fascicoli SIUS pendenti " from avvocato a JOIN AVVOCATO_FASCICOLO_SIUS afs ON (afs.AVV_ID_AVVOCATO = a.ID_AVVOCATO AND SUBSTR(afs.FAS_SIU_ID_FASCICOLO_SIUS,-6, 2) = 'NN')
JOIN FASCICOLO_SIUS fs ON (fs.ID_FASCICOLO_SIUS = afs.FAS_SIU_ID_FASCICOLO_SIUS
and  fs.COD_STATO_FASCICOLO  NOT IN ('01','05','07') )
...
Accedendo, inoltre, alla tabella PRE_XBA_AVVOCATI_CON_PENDENZE è possibile selezionare, a campione, procedimenti SIEP, SIUS e SIGE e il relativo avvocato assegnatario: collegandosi, quindi, da applicativo web, è possibile verificare che i procedimenti risultino effettivamente in uno stato di PENDENZA ed assegnati all’avvocato indicato in tabella.
## 3.2.2 STEP 2
Verificare tramite il comando pwd di trovarsi nel percorso /home/oracle/MEV-2019_21/bonifica_db/ ed eseguire il comando
...
SELECT foro, COUNT(*) FROM AVVOCATO_SXBA WHERE FORO IN ('REGGIO DI CALABRIA', 'REGGIO CALABRIA', 'REGGIO NELL''EMILIA', 'REGGIO EMILIA', 'MASSA', 'CARRARA', 'MASSA CARRARA', 'FORLI'' CESENA', 'FORLI''', 'CESENA', 'FORLÌ-CESENA')  GROUP BY foro
...
Tabella pre aggiornamento

Query sulla tabella AVVOCATO, oggetto delle modifiche:

SELECT foro, COUNT(*) FROM AVVOCATO WHERE FORO IN ('REGGIO DI CALABRIA', 'REGGIO CALABRIA', 'REGGIO NELL''EMILIA', 'REGGIO EMILIA', 'MASSA', 'CARRARA', 'MASSA CARRARA', 'FORLI'' CESENA', 'FORLI''', 'CESENA', 'FORLÌ-CESENA') GROUP BY foro
...
Tabella post aggiornamento

verificare che nella tabella post aggiornamento:
siano presenti solo i fori in essa riportati e che il totale del numero di records sia uguale al totale dei records riportati nella Tabella pre;
che il totale dei records per FORLÌ-CESENA = somma dei records riportati per FORLI’, CESENA, FORLI’ CESENA, FORLÌ-CESENA della Tabella pre, in cui possono essere presenti solo parte delle precedenti descrizioni;
che il totale dei records per MASSA CARRARA = somma dei records riportati per MASSA, CARRARA, MASSA CARRARA della Tabella pre;
che il totale dei records per REGGIO CALABRIA = somma dei records REGGIO CALABRIA, REGGIO DI CALABRIA della Tabella pre;
che il totale dei records per REGGIO EMILIA = somma dei records REGGIO EMILIA, REGGIO NELL’ EMILIA della Tabella pre

Si evidenzia che la bonifica riguarderà solo gli avvocati con procedimenti pendenti appartenenti a fori riconosciuti da SIES/ReGIndE. Per estrarre gli avvocati con procedimenti pendenti ma con fori non riconosciuti, eseguire la seguente query, in cui bisogna sostituire il parametro ‘NN’ con il Codice del Distretto.

SELECT DISTINCT a.* FROM AVVOCATO a
JOIN AVVOCATO_FASCICOLO_SIEP afs ON (afs.AVV_ID_AVVOCATO = a.ID_AVVOCATO and SUBSTR(afs.FAS_SIE_ID_FASCICOLO_SIEP,-6, 2) = 'NN')
JOIN FASCICOLO_SIEP fs ON (fs.ID_FASCICOLO_SIEP = afs.FAS_SIE_ID_FASCICOLO_SIEP
and  fs.COD_STATO_FASCICOLO <> '01') WHERE a.FORO NOT IN (SELECT RV_MEANING FROM CG_REF_CODES crc WHERE RV_DOMAIN = 'FORO_AVVOCATI'AND RV_MEANING = FORO)
UNION
SELECT DISTINCT a.* FROM AVVOCATO a
JOIN AVVOCATO_FASCICOLO_SIUS afs ON (afs.AVV_ID_AVVOCATO = a.ID_AVVOCATO and SUBSTR(afs.FAS_SIU_ID_FASCICOLO_SIUS,-6, 2) = 'NN')
JOIN FASCICOLO_SIUS fs ON (fs.ID_FASCICOLO_SIUS = afs.FAS_SIU_ID_FASCICOLO_SIUS
and  fs.COD_STATO_FASCICOLO NOT IN ('01','05','07')) WHERE a.FORO NOT IN (SELECT RV_MEANING FROM CG_REF_CODES crc WHERE RV_DOMAIN = 'FORO_AVVOCATI' AND RV_MEANING = FORO)
UNION
SELECT DISTINCT a.* FROM AVVOCATO a
JOIN AVVOCATO_FASCICOLO_SIGE afs ON (afs.AVV_ID_AVVOCATO = a.ID_AVVOCATO and SUBSTR(afs.FAS_SIGE_ID_FASCICOLO_SIGE,-6, 2) = 'NN')
JOIN FASCICOLO_SIGE fs ON (fs.ID_FASCICOLO_SIGE = afs.FAS_SIGE_ID_FASCICOLO_SIGE
and  fs.DATA_DEFINIZIONE IS NULL) WHERE a.FORO NOT IN (SELECT RV_MEANING FROM CG_REF_CODES crc WHERE RV_DOMAIN = 'FORO_AVVOCATI' AND RV_MEANING = FORO)

il risultato deve essere salvato in un file.xls e conservato per eventuali future verifiche.

Se si vogliono anche i singoli procedimenti pendenti a carico di ciascuno dei suddetti avvocati bisogna eseguire la seguente query

SELECT DISTINCT xacp.* FROM PRE_XBA_AVVOCATI_CON_PENDENZE xacp
WHERE FORO NOT IN (SELECT RV_MEANING FROM CG_REF_CODES_SXBA crc WHERE RV_DOMAIN = 'FORO_AVVOCATI' AND RV_MEANING = FORO)
AND FORO NOT IN ('REGGIO DI CALABRIA', 'REGGIO CALABRIA','REGGIO NELL''EMILIA', 'REGGIO EMILIA', 'MASSA',  'CARRARA',  'MASSA CARRARA', 'FORLI'' CESENA', 'FORLI''', 'CESENA', 'FORLÌ-CESENA') ORDER BY foro,id_avvocato, id_fascicolo_siep
## 3.2.3 STEP 3
Verificare tramite il comando pwd di trovarsi nel percorso /home/oracle/MEV-2019_21/bonifica_db/ ed eseguire il comando
...
Dettaglio del file:
XBA_CREA_AVV_CON_PENDENZE.prc = Crea la tabella XBA_AVVOCATI_CON_PENDENZE, che conterrà i dati di transito per poter eseguire la bonifica Avvocato SIES. La tabella è una immagine della tabella AVVOCATO con l’aggiunta di 3 nuove colonne AMBIENTE, NUM_PENDENZE, ID_AVV_CERT_REGINDE. La tabella strutturalmente è una immagine della tabella AVVOCATO, con l’aggiunta delle seguenti colonne:
...
1. Verificare che il numero di records presenti nella tabella XBA_AVVOCATI_CON_PENDENZE, avvocati con procedimenti pendenti, ottenuto   eseguendo la seguente query

SELECT count(DISTINCT ID_AVVOCATO) FROM XBA_AVVOCATI_CON_PENDENZE

sia uguale al numero di difensori presente nella tabella PRE_XBA_AVVOCATI_CON_PENDENZE, ottenuto eseguendo la seguente query

SELECT count(DISTINCT ID_AVVOCATO) FROM PRE_XBA_AVVOCATI_CON_PENDENZE

Verificare anche che il totale dei procedimenti pendenti, calcolati nella tabella XBA_AVVOCATI_CON_PENDENZE, ottenuto   eseguendo la seguente query

SELECT sum(num_pendenze) FROM XBA_AVVOCATI_CON_PENDENZE

sia uguale al numero di procedimenti pendenti, presenti nella tabella PRE_XBA_AVVOCATI_CON_PENDENZE, ottenuto eseguendo la seguente query

SELECT count(*) FROM PRE_XBA_AVVOCATI_CON_PENDENZE
## 3.2.4 STEP 4
Verificare tramite il comando pwd di trovarsi nel percorso /home/oracle/MEV-2019_21/bonifica_db/ ed eseguire il comando
...
eseguire nell’ordine le seguenti quattro queries:

SELECT count(*) "Totale avv. con proc. pendenti" FROM XBA_AVVOCATI_CON_PENDENZE;

SELECT count(*) "Totale avv. certificabili" FROM XBA_AVVOCATI_CON_PENDENZE WHERE FLAG_REGINDE = 'SI';

SELECT count(*) "Tot.avv. assoc. a certif." FROM XBA_AVVOCATI_CON_PENDENZE WHERE FLAG_REGINDE = 'NO' AND ID_AVV_CERT_REGINDE IS NOT NULL;

SELECT count(*) "Tot.avv. non certificabili" FROM XBA_AVVOCATI_CON_PENDENZE WHERE FLAG_REGINDE = 'NO' AND ID_AVV_CERT_REGINDE IS NULL;

verificare che i numeri forniti soddisfino la seguente espressione:

Totale avv. con proc. pendenti = Totale avv. certificabili + Tot.avv. assoc. a certif. + Tot.avv. non certificabili

E’ inoltre possibile accedere ai dati della vista XBA_VERIF_AVV_CERTIFICABILI, eseguendo la query:
...
Selezionare a campione dalla lista il codice fiscale di un avvocato, eseguire la query di seguito riportata, sostituendo il codice fiscale XXXXXXXXXXXXXXXX con quello selezionato
...
Il cursore così individua prima le occorrenza di AVVOCATO che saranno certificati REGINDE e subito dopo le occorrenze collegate allo stesso AVVOCATO ( aventi lo stesso ID_AVV_CERT_REGINDE ) per i quali si procederà a modificare l’AVV_ID_AVVOCATO nelle tabelle di relazione.
...
che conterrà i dettagli di tutti gli avvocati aventi procedimenti SIEP, SIUS e SIGE pendenti dopo la bonifica ed è finalizzata a permettere di verificarne la corretta esecuzione. La tabella strutturalmente è una immagine della tabella PRE_XBA_AVVOCATI_CON_PENDENZE, con l’aggiunta della colonna FLAG_REGINDE.
...
per effetto dell’accorpamento degli avvocati a quelli certificati restituisca un numero <= a quello restituito dalla query:

SELECT count(DISTINCT ID_AVVOCATO) FROM PRE_XBA_AVVOCATI_CON_PENDENZE pacp WHERE
ambiente = 'SIEP'
...
ordinandola per codice fiscale, selezionare un avvocato con un numero corposo di pendenze, ed eseguire la query cambiando ‘XXXXXXXXXXXXXXXX’ con il codice fiscale dell’avvocato selezionato

SELECT count(*) FROM POST_XBA_AVVOCATI_CON_PENDENZE WHERE COD_FISCALE = 'XXXXXXXXXXXXXXXX'

Eseguire poi la seguente query con lo stesso codice fiscale

SELECT count(*)  FROM PRE_XBA_AVVOCATI_CON_PENDENZE WHERE COD_FISCALE = 'XXXXXXXXXXXXXXXX'

verificare che il numero di records estratti sia corrispondente a quello dei records estratti sulla tabella POST.

E’ possibile verificare anche che il suddetto numero corrisponda alla somma dei records distinti per i tre ambienti SIEP, SIUS e SIGE sia sulla tabella POST_XBA_AVVOCATI_CON_PENDENZE sia sulla tabella PRE_XBA_AVVOCATI_CON_PENDENZE eseguendo le seguenti queries:
SELECT SUM(fas_siep),SUM(fas_sius), SUM(fas_sige) FROM (SELECT COUNT(a.id_fascicolo_siep) fas_siep, 0 fas_sius, 0 fas_sige FROM POST_XBA_AVVOCATI_CON_PENDENZE a WHERE a.cod_fiscale='XXXXXXXXXXXXXXXX' AND a.ambiente='SIEP'
AND a.id_fascicolo_siep IS NOT NULL
UNION
SELECT 0 fas_siep, COUNT(a.id_fascicolo_sius) fas_sius, 0 fas_sige FROM POST_XBA_AVVOCATI_CON_PENDENZE a WHERE a.cod_fiscale='XXXXXXXXXXXXXXXX'
AND a.ambiente='SIUS'
AND a.id_fascicolo_sius IS NOT NULL
UNION
SELECT 0 fas_siep, 0 fas_sius, COUNT(a.id_fascicolo_sige) fas_sige FROM POST_XBA_AVVOCATI_CON_PENDENZE a WHERE a.cod_fiscale='XXXXXXXXXXXXXXXX'
AND a.ambiente='SIGE'
AND a.id_fascicolo_sige IS NOT NULL);

SELECT SUM(fas_siep),SUM(fas_sius), SUM(fas_sige) FROM (SELECT COUNT(a.id_fascicolo_siep) fas_siep, 0 fas_sius, 0 fas_sige FROM PRE_XBA_AVVOCATI_CON_PENDENZE a WHERE a.cod_fiscale='XXXXXXXXXXXXXXXX' AND a.ambiente='SIEP'
AND a.id_fascicolo_siep IS NOT NULL
UNION
SELECT 0 fas_siep, COUNT(a.id_fascicolo_sius) fas_sius, 0 fas_sige FROM 				PRE_XBA_AVVOCATI_CON_PENDENZE a WHERE a.cod_fiscale='XXXXXXXXXXXXXXXX'
AND a.ambiente='SIUS'
AND a.id_fascicolo_sius IS NOT NULL
UNION
SELECT 0 fas_siep, 0 fas_sius, COUNT(a.id_fascicolo_sige) fas_sige FROM PRE_XBA_AVVOCATI_CON_PENDENZE a WHERE a.cod_fiscale='XXXXXXXXXXXXXXXX'
AND a.ambiente='SIGE'
AND a.id_fascicolo_sige IS NOT NULL);

Con le seguenti queries è possibile estrarre l’elenco dei procedimenti pendenti dei diversi ambienti dalle tabelle PRE e POST Bonifica, sostituendo ‘XXXXXXXXXXXXXXXX’ con il codice fiscale selezionato e ‘NN’ con il codice del Distretto.

Il numero dei procedimenti pendenti SIEP deve essere = SUM(FAS_SIEP):

SELECT
fs.chiave_anno, fs.chiave_progr, u.DESCR_TIPO_UFFICIO, u.DESCR_COMUNE, fs.COD_STATO_FASCICOLO, ID_AVVOCATO, FLAG_REGINDE
FROM AVVOCATO a
JOIN AVVOCATO_FASCICOLO_SIEP afs ON (afs.AVV_ID_AVVOCATO = a.ID_AVVOCATO and SUBSTR(afs.FAS_SIE_ID_FASCICOLO_SIEP,-6, 2) = 'NN')
JOIN FASCICOLO_SIEP fs ON (fs.ID_FASCICOLO_SIEP = afs.FAS_SIE_ID_FASCICOLO_SIEP and  fs.COD_STATO_FASCICOLO <> '01' )
JOIN UFFICIO_DESCR u ON (fs.CHIAVE_UFFICIO = u.COD_UFFICIO)
WHERE a.cod_fiscale = 'XXXXXXXXXXXXXXXX'
ORDER BY id_avvocato, u.descr_TIPO_UFFICIO, u.descr_comune, fs.CHIAVE_ANNO, fs.CHIAVE_PROGR;

SELECT
fs.chiave_anno, fs.chiave_progr, u.DESCR_TIPO_UFFICIO, u.DESCR_COMUNE, fs.COD_STATO_FASCICOLO, ID_AVVOCATO
FROM AVVOCATO_SXBA a
JOIN AVVOCATO_FASCICOLO_SIEP_SXBA afs ON (afs.AVV_ID_AVVOCATO = a.ID_AVVOCATO and SUBSTR(afs.FAS_SIE_ID_FASCICOLO_SIEP,-6, 2) = 'NN')
JOIN FASCICOLO_SIEP fs ON (fs.ID_FASCICOLO_SIEP = afs.FAS_SIE_ID_FASCICOLO_SIEP and  fs.COD_STATO_FASCICOLO <> '01' )
JOIN UFFICIO_DESCR u ON (fs.CHIAVE_UFFICIO = u.COD_UFFICIO)
WHERE a.cod_fiscale = 'XXXXXXXXXXXXXXXX'
ORDER BY id_avvocato, u.descr_TIPO_UFFICIO, u.descr_comune, fs.CHIAVE_ANNO, fs.CHIAVE_PROGR;

Il numero dei procedimenti pendenti SIUS deve essere = SUM(FAS_SIUS)

SELECT
fs.chiave_anno, fs.chiave_progr, u.DESCR_TIPO_UFFICIO, u.DESCR_COMUNE, fs.COD_STATO_FASCICOLO, ID_AVVOCATO, FLAG_REGINDE
FROM AVVOCATO a
JOIN AVVOCATO_FASCICOLO_SIUS afs ON (afs.AVV_ID_AVVOCATO = a.ID_AVVOCATO and SUBSTR(afs.FAS_SIU_ID_FASCICOLO_SIUS,-6, 2) = 'NN')
JOIN FASCICOLO_SIUS fs ON (fs.ID_FASCICOLO_SIUS = afs.FAS_SIU_ID_FASCICOLO_SIUS and  fs.COD_STATO_FASCICOLO NOT in ('01', '07', '05'))
JOIN UFFICIO_DESCR u ON (fs.CHIAVE_UFFICIO = u.COD_UFFICIO)
WHERE a.cod_fiscale = 'XXXXXXXXXXXXXXXX'
ORDER BY id_avvocato, u.descr_TIPO_UFFICIO, u.descr_comune, fs.CHIAVE_ANNO, fs.CHIAVE_PROGR;
...
fs.chiave_anno, fs.chiave_progr, u.DESCR_TIPO_UFFICIO, u.DESCR_COMUNE, fs.COD_STATO_FASCICOLO, ID_AVVOCATO
FROM AVVOCATO_SXBA a
JOIN AVVOCATO_FASCICOLO_SIUS_SXBA afs ON (afs.AVV_ID_AVVOCATO = a.ID_AVVOCATO and SUBSTR(afs.FAS_SIU_ID_FASCICOLO_SIUS,-6, 2) = 'NN')
JOIN FASCICOLO_SIUS fs ON (fs.ID_FASCICOLO_SIUS = afs.FAS_SIU_ID_FASCICOLO_SIUS and  fs.COD_STATO_FASCICOLO NOT in ('01', '07', '05'))
JOIN UFFICIO_DESCR u ON (fs.CHIAVE_UFFICIO = u.COD_UFFICIO)
WHERE a.cod_fiscale = 'XXXXXXXXXXXXXXXX'
ORDER BY id_avvocato, u.descr_TIPO_UFFICIO, u.descr_comune, fs.CHIAVE_ANNO, fs.CHIAVE_PROGR;

Il numero dei procedimenti pendenti SIGE deve essere = SUM(FAS_SIGE)

SELECT
fs.chiave_anno, fs.chiave_progr, u.DESCR_TIPO_UFFICIO, u.DESCR_COMUNE, fs.COD_STATO_FASCICOLO, ID_AVVOCATO, FLAG_REGINDE
FROM AVVOCATO a
JOIN AVVOCATO_FASCICOLO_SIGE afs ON (afs.AVV_ID_AVVOCATO = a.ID_AVVOCATO and SUBSTR(afs.FAS_SIGE_ID_FASCICOLO_SIGE,-6, 2) = 'NN')
JOIN FASCICOLO_SIGE fs ON (fs.ID_FASCICOLO_SIGE = afs.FAS_SIGE_ID_FASCICOLO_SIGE and  fs.DATA_DEFINIZIONE IS NULL )
JOIN UFFICIO_DESCR u ON (fs.CHIAVE_UFFICIO = u.COD_UFFICIO)
WHERE a.cod_fiscale = 'XXXXXXXXXXXXXXXX'
ORDER BY id_avvocato, u.descr_TIPO_UFFICIO, u.descr_comune, fs.CHIAVE_ANNO, fs.CHIAVE_PROGR;
...
fs.chiave_anno, fs.chiave_progr, u.DESCR_TIPO_UFFICIO, u.DESCR_COMUNE, fs.COD_STATO_FASCICOLO, ID_AVVOCATO
FROM AVVOCATO_SXBA a
JOIN AVVOCATO_FASCICOLO_SIGE_SXBA afs ON (afs.AVV_ID_AVVOCATO = a.ID_AVVOCATO and SUBSTR(afs.FAS_SIGE_ID_FASCICOLO_SIGE,-6, 2) = 'NN')
JOIN FASCICOLO_SIGE fs ON (fs.ID_FASCICOLO_SIGE = afs.FAS_SIGE_ID_FASCICOLO_SIGE and  fs.DATA_DEFINIZIONE IS NULL )
JOIN UFFICIO_DESCR u ON (fs.CHIAVE_UFFICIO = u.COD_UFFICIO)
WHERE a.cod_fiscale = 'XXXXXXXXXXXXXXXX'
ORDER BY id_avvocato, u.descr_TIPO_UFFICIO, u.descr_comune, fs.CHIAVE_ANNO, fs.CHIAVE_PROGR;
...
Attenzione: le verifiche da applicativo riportate di seguito potranno essere effettuate solo al termine di tutte le attività previste nel piano di rilascio.
...
Per verificare il corretto aggiornamento del campo ‘avv_id_avvocato’ dei procedimenti SIEP pendenti, eseguire la seguente query:
SELECT * FROM XBA_VERIF_AVV_FAS_SIEP;
dalla lista selezionare a campione dei procedimenti e verificare da applicativo, che al procedimento risultino collegati avvocati certificati.

Per verificare il corretto aggiornamento del campo ‘avv_id_avvocato’ dei procedimenti SIGE pendenti, eseguire la seguente query:
SELECT * FROM XBA_VERIF_AVV_FAS_SIGE;
dalla lista selezionare a campione dei procedimenti e verificare da applicativo, che al procedimento risultino collegati avvocati certificati.

Per verificare il corretto aggiornamento del campo ‘avv_id_avvocato’ dei procedimenti SIUS pendenti, eseguire la seguente query:
SELECT * FROM XBA_VERIF_AVV_FAS_SIUS;
dalla lista selezionare a campione dei procedimenti e verificare da applicativo, che al procedimento risultino collegati avvocati certificati.

Per verificare il corretto aggiornamento del campo ‘avv_id_avvocato’ dei records AVVISI_AVVOCATO, eseguire la seguente query:
SELECT * FROM XBA_VERIF_AVVISI_AVVOCATO;
dalla lista selezionare a campione alcuni records, per ciascuno annotarsi il valore (NNNNNNN) della colonna ID_AVVISO ed eseguire le seguenti queries

SELECT a.ID_AVVOCATO, COD_FISCALE, COGNOME, NOME, FORO, FLAG_REGINDE
FROM AVVOCATO a, AVVISI_AVVOCATO aa WHERE ID_AVVISO = NNNNNNN
AND a.ID_AVVOCATO = aa.ID_AVVOCATO

che visualizza i dati anagrafici principali dell’avvocato, collegato all’avviso e aggiornato a seguito della bonifica, verificare che FLAG_REGINDE = ‘SI’

SELECT a.ID_AVVOCATO, COD_FISCALE, COGNOME, NOME, FORO, FLAG_REGINDE
FROM AVVOCATO a, AVVISI_AVVOCATO_SXBA aa WHERE ID_AVVISO = NNNNNNN
AND a.ID_AVVOCATO = aa.ID_AVVOCATO

che visualizza i dati anagrafici principali dell’avvocato, collegato all’avviso prima della bonifica,  verificare che FLAG_REGINDE = ‘NO’.

Per verificare il corretto aggiornamento del campo ‘avv_id_avvocato’ dei records PARTI_UDIENZA_DIFENSORE, eseguire la seguente query:
SELECT * FROM XBA_VERIF_PARTI_UDIENZA_DIF;

dalla lista selezionare a campione alcuni records, per ciascuno annotarsi il valore (NNNNNNN) della colonna ID_AVVOCATO_PARTE_UDIENZA ed eseguire le seguenti queries

SELECT a.ID_AVVOCATO, COD_FISCALE, COGNOME, NOME, FORO, FLAG_REGINDE
FROM AVVOCATO a, PARTI_UDIENZA_DIFENSORE pud
WHERE ID_AVVOCATO_PARTE_UDIENZA = NNNNNNN
AND a.ID_AVVOCATO = pud.AVV_ID_AVVOCATO

che visualizza i dati anagrafici principali dell’avvocato, collegato a PARTE_UDIENZA_DIFENSORE e aggiornato a seguito della bonifica,   verificare che FLAG_REGINDE = ‘SI’
...
che visualizza i dati anagrafici principali dell’avvocato, collegato a PARTE_UDIENZA_DIFENSORE prima della bonifica,  verificare che FLAG_REGINDE = ‘NO’.

Per verificare il corretto aggiornamento del campo ‘avv_id_avvocato’ dei records STORICO_AVVOCATO, eseguire la seguente query:
SELECT * FROM XBA_VERIF_STORICO_AVVOCATO;

dalla lista selezionare a campione alcuni records, per ciascuno annotarsi il valore (NNNNNNN) della colonna ID_STORICO_AVVOCATO ed eseguire le seguenti queries

SELECT a.ID_AVVOCATO, a.COD_FISCALE, a.COGNOME, a.NOME, a.FORO, FLAG_REGINDE
FROM AVVOCATO a, STORICO_AVVOCATO sa   WHERE ID_STORICO_AVVOCATO = NNNNNNN
AND a.ID_AVVOCATO = sa.AVV_ID_AVVOCATO

che visualizza i dati anagrafici principali dell’avvocato, collegato a STORICO_AVVOCATO e aggiornato a seguito della bonifica,   verificare che FLAG_REGINDE = ‘SI’

SELECT a.ID_AVVOCATO, a.COD_FISCALE, a.COGNOME, a.NOME, a.FORO, FLAG_REGINDE
FROM AVVOCATO a, STORICO_AVVOCATO_SXBA sa   WHERE ID_STORICO_AVVOCATO = NNNNNNN
AND a.ID_AVVOCATO = sa.AVV_ID_AVVOCATO

che visualizza i dati anagrafici principali dell’avvocato, collegato a STORICO_AVVOCATO prima della bonifica,  verificare che FLAG_REGINDE = ‘NO’.

Per verificare il corretto aggiornamento del campo ‘avv_id_avvocato’ e del campo ‘avv_id_avvocato_presentante’ dei records NUOVA_ISTANZA, eseguire la seguente query:
SELECT * FROM XBA_VERIF_NUOVA_ISTANZA WHERE AVV_ID_AVVOCATO_NEW IS NOT NULL AND AVV_ID_AVVPRES_NEW IS NOT null;

(N.B. poiché la vista contiene anche records per cui è stato aggiornato solo uno dei due riferimenti all’avvocato, è possibile effettuarne l’estrazione modificando le condizioni nella precedente query).

Dalla lista selezionare a campione alcuni records, per ciascuno annotarsi il valore (NNNNNNN) della colonna ID_NUOVA_ISTANZA ed eseguire le seguenti queries:

SELECT a.ID_AVVOCATO, a.COD_FISCALE, a.COGNOME, a.NOME, a.FORO, a.FLAG_REGINDE,
aa.ID_AVVOCATO "AVVOCATO_PRESENTANTE", aa.COD_FISCALE, aa.COGNOME, aa.NOME, aa.FORO,aa.FLAG_REGINDE
FROM AVVOCATO a, AVVOCATO aa,NUOVA_ISTANZA ni  WHERE ID_NUOVA_ISTANZA = NNNNNNN
AND a.ID_AVVOCATO = ni.AVV_ID_AVVOCATO AND aa.ID_AVVOCATO = ni.AVV_ID_AVVOCATO_PRESENTANTE

che visualizza i dati anagrafici principali degli avvocati, collegati a NUOVA_ISTANZA e aggiornati a seguito della bonifica,   verificare che FLAG_REGINDE = ‘SI’
...
che visualizza i dati anagrafici principali degli avvocati, collegati a NUOVA_ISTANZA prima della bonifica,  verificare che FLAG_REGINDE = ‘NO’.
...
Alla fine di queste attività di aggiornamento, passare paragrafo 6.3 del documento “SIUT-SIES-PR-1.6-20221125-Piano_di_rilascio_021_RegInde_SIES.pdf”.
## 3.2.7 STEP 7
In caso di errori verificatisi nell’esecuzione di uno degli step precedenti, a seguito indicazioni dell’help desk del fornitore, è necessario ripristinare lo snapshot effettuato all’inizio della procedura.